...
const TutorialPaymentSuccess = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [status, setStatus] = useState('checking');
  const [message, setMessage] = useState('Confirming your tutorial payment...');
  ...
  useEffect(() => {
    ...
    if (!reference) {
      setStatus('error');
      setMessage('Missing payment reference. We could not verify your tutorial payment.');
      return;
    }
    ...
    const confirmPayment = async () => {
      try {
        const res = await apiService.confirmTutorialPaynowEnrollment(reference);
        const payload = res?.data || res;

        if (payload?.paymentStatus === 'paid' && payload?.tutorialId) {
          navigate(`/tutorials/${payload.tutorialId}`, { replace: true });
          return;
        }
        ...
        if (!isActive) return;
        setStatus('failed');
        setMessage('Your tutorial payment was not completed, so enrollment has not been activated.');
      } catch (error) {
        if (!isActive) return;
        setStatus('error');
        setMessage(error.message || 'We could not confirm your tutorial payment.');
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-12">
      <div className="mx-auto max-w-xl rounded-[2rem] bg-white p-10 text-center shadow-[0_25px_75px_-35px_rgba(15,23,42,0.2)]">
        ...
        <h1 className="mt-6 text-3xl font-bold text-slate-900">Tutorial Payment</h1>
...
export default TutorialPaymentSuccess;
